...
class HogeScene {
    constructor(pScene) {
        ...
        this.customerEnterPos = Vector2(-40, 48);
        this.enemyDataDict = {
            'enm_1': {
                speed: 25,
                scoreScale: 1,
    ...
    getGameObject(goId) {
        var go = this.goArr.find((_elem) => {
            if (_elem.instanceId !== goId)
                return false;
            return true;
        });
        return go;
    }
    findGameObject(goId, func = null) {
        var go = this.goArr.find((_elem) => {
            if (_elem.instanceId !== goId)
                return false;
            return true;
        ...
        if (self.lastPointBtn !== null) {
            ...
            if (lastGO.customer !== null) {
                ...
                const customerOnChair = self.goArr.find((_elem) => {
                    if (!_elem.customer)
                        return false;
                    if (_elem.customer.chairId !== lastGO.instanceId)
                        return false;
                    return true;
                });
                const selectedCustomer = self.goArr.find((_elem) => {
                    if (!_elem.customer)
                        return false;
                    if (_elem.instanceId !== player.selectedCustomerId)
                        return false;
                    return true;
                });
                ...
                var noodleGO = self.goArr.find((_go) => {
                    if (_go.instanceId !== player.selectedNoodleId)
                        return false;
                    return true;
                });
                if (noodleGO && noodleGO.noodle) {
                    ...
                    const bankGO = self.goArr.find((_elem) => {
                        if (_elem.instanceId !== player.selectedBankId)
                            return false;
                        return true;
                    });
                    player.selectedNoodleId = 0;
                ...
                var noodleInPot = self.goArr.find((_go) => {
                    if (_go.noodle === null)
                        return false;
                    if (_go.noodle.ownerId !== potGo.instanceId)
                        return false;
                    return true;
                });
                if (bankGO) {
                ...
                switch (waveType) {
                    case 1: {
                        const waitingCustomerArr = this.findComponents((_go) => {
                            if (_go.customer === null)
                                return null;
                            if (_go.customer.waitIndex === -1)
                                return null;
                            return _go.customer;
                        });
    ...
    static boilLvToTime(boilLv) {
        return boilLv * 5000;
    }
    static boilTimeToTimerRotation(boilTime) {
            ...
            switch (customer.state) {
                ...
                case CustomerState.STAND_WAIT:
                    ...
                    }
                    {
                        ...
                        const noodleArr = self.findComponents((_go2) => {
                            if (_go2.noodle === null)
                                return null;
                            if (_go2.noodle.ownerId !== _go.instanceId)
                                return null;
                            return _go2.noodle;
                        });
                    ...
                    if (0 < customer.waitIndex) {
                        ...
                        for (var i = 0; i < customerArr.length; i++) {
                            ...
                            if (otherCustomer.waitIndex !== nextIndex)
                                continue;
                            hasNextIndex = false;
                            break;
                        }
                ...
                case CustomerState.MOVE: {
                    ...
                    const chairGO = self.goArr.find((_elem) => {
                        if (_elem.instanceId !== customer.chairId)
                            return false;
                        return true;
                    });
                    ...
                    if (1 <= progress) {
                        ...
                        const trayGO = self.goArr.find((_elem) => {
                            if (!_elem.tray)
                                return false;
                            if (_elem.tray.chairId !== chairGO.instanceId)
                                return false;
                            return true;
                        });
                ...
                case CustomerState.EAT: {
                    let noodle = self.goArr.find((_elem) => {
                        if (!_elem.noodle)
                            return false;
                        return _elem.noodle.ownerId === customer.trayId;
                    });
        ...
        self.goArr.forEach((_go) => {
            if (_go.noodle) {
                ...
                const onwerGO = self.goArr.find((_elem) => {
                    if (_elem.instanceId !== noodle.ownerId)
                        return false;
                    return true;
                });
                if (onwerGO === null) {
